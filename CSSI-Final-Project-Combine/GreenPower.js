...
async function printData() {
  ...
  const query = `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=GPV.TRV&outputsize=full&apikey=${key}`;
...
calculateButton.addEventListener("click", e => {
  ...
  async function addDate() {
    ...
    let time = timeData["Time Series (Daily)"];
     for (const item in time) {
       count++;
       if(count===finalStart) date.push(item);
       if(count===finalEnd) date.push(item);
     }
    date = date.reverse();
  }
  ...
  addDate().then(timeData => {
      message.innerHTML = `<section><div style="display: inline;"> The best day to buy your stock is day </div> <div style="display: inline;" class="has-text-success">${date[0]}</div></section><section><div style="display: inline;">The best day to sell your stock is day </div><div style="display: inline;" class="has-text-success">${date[1]}</div></section>`;
